...
`snakemake` [@Koster2012-sq; @Molder2021-mp] or `nextflow` [@Di_Tommaso2017-yo].
...
other resources like computational environments (Table 1).
...
| Item | Location |
| --- | --- |
| Data set and metadata | Persistent repository |
| Code base and documentation | DVCS (eg: GitHub) and snapshot deposited to persistent repository |
| Container image | Container registry and persistent repository |

: Table 1. Supporting information for a five-pillar research article.
...
As outlined above, one of the goals is to reduce the complexity of reproduction, using a master script.
...
In addition, the `README` should outline how users can contribute to the codebase and how to report
issues, such as code errors and discrepancies.
...
so that machines or individuals browsing these resources can understand the context of the overall
research project.
...
reference genome and variant calling which are demanding in memory and computaion.
