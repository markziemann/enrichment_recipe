...
```{r,dvcs,fig.cap="Figure 3. Distributed version control allows. Adapted from @Chacon2014-ys.",echo=FALSE,fig.height=6}
...
In particular, when large research projects are funded by the public, the public might expect that the
benefts of that study, like the publications and underlying data are made publicly accessible.

Data sharing has dual purposes, firstly to facilitate reproduction and auditing, and secondly to
enable reuse in other contexts [@Wilkinson2016-qq].
Data reuse increases efficiency, as it prevents redundant research and facilitates new research ideas
that were impossible without it.
In terms of research rigour, inspecting raw data can uncover research integrity problems [@Miyakawa2020-yj],
in addition to validating scientific findings by independent computational reproduction [@Ioannidis2009-hv].
...
The field of genomics has has a long history of data sharing @Brazma2001-yk, which is supported by
...
conforming to FAIR principles (eg: GEO, SRA, PRIDE [@Edgar2002-hs; @Kodama2012-bn; @Deutsch2017-hc]).
...
2. If possible, avoid large supplementary data files accompanying journal articles.

2. Provide detailed metadata.

3. Preferably archive and share raw data, and use existing standards for the discipline.
...
This ensures code and data are linked, which has been a  [@Peng2011-ia]
