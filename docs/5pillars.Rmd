...
share parts of the host operating system (**Figure 4**).
In contrast to VMs, running containers incurs only a small reduction in performance as compared
...
![Figure 4. Architecture of containers (left) compared to virtual machines (right)](../img/docker-containerized-and-vm-transparent-bg.webp)
...
Docker images are larger, and cannot be shared in the code repository, so these need to be archived
in a long term repository, such as Zenodo or FigShare.
...
When these features are brought together they facilitate extreme reproducibility, ie: makes reproducing a
previous study relatively easy in terms of complexity, number of commands, and in time.
For example, we made an example transcriptome analysis workflow based on the five pillars approach and
it could could be installed and generate differential expression and pathway enrichment results in a
matter of minutes @Ziemann_undated-zx.
...
made to the project's code base @Beaulieu-Jones2017-vj.
...
"temple" of reproducibl research.
