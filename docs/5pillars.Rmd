...
@Zaringhalam2020-ms.
A recent systamatic analysis of Jupyter notebooks in biomedical articles had similar observations, with
only 245/4169 (5.9%) notebooks giving similar results compared to the original, with irreproducibility
...
This article, inspired by Peikert and BrandMaier @Peikert2021-eq, attempts to unify these concepts into
five key principles called the five pillars of reproducible computational research (**Figure 1**).
...
The advantage of such tools is that if the analysis were to fail mid-way,
for example at step 8 of a 15-step workflow, the analysis of steps 1-7 wouldn't need to be repeated.
After fixing the issue with code at step 8, re-running the analysis would pick up at step 8 again,
...
Similarly, the [Streamlit](https://streamlit.io/) tool, which was originally invented for creating
Python-based web dashboards but is equally capable of generating data science reports
[e.g.: @Nantasenamat2023-wr].
...
This means it is possible to author an entire journal articles using literate programming.
...
require certain system libraries for low-level routines.
...
For example, it is now possible to embed a Wasm-enabled R terminal inside a Quarto HTML document, which
...
6. Use file formats that are compatible with many different types of software; e.g.: comma-separated
...
Typically it is written in the markdown format, which should be familiar
...
Part of the problem is the incentive structures in academia, with funding bodies and journals are
focused on novel research directions rather than confirming previous findings
[@Kaelin2017-id; @Munafo2017-ty].
