...
A common recommendation for reproducibility is to complete all works with computer scripts
and remove the need for any manual steps.
Computer programs (scripts) are preferred over graphical interface tools as the script contains
a complete instruction set for how a piece of work was conducted.
This allows auditing and reproduction which would be difficult for graphical tools like spreadsheets.
Scripts also allow the work to be completely automated and reduce the chance for human error during
any manual steps.
On the other hand, data cleaning and curation is sometimes required, and so manual steps are unavoidable
in some situations.

Literate programming combines "chunks" of analytical code together with human readable text @Knuth1984-di.
After compilation, the resulting output contains the code with computational results
(eg: figures and tables) along with contextualising explanations and narratives.
...
@Leisch2002-yc.
Around 2015, R Markdown emerged as a more popular alternative, as formatting is simpler as compared to Sweave,
saving time.
...
In 2022, the company behind RStudio (Posit) released Quarto, the conceptual successor of R Markdown, but with
enhanced support for other computer languages like Python, Julia, and Observable JavaScript
(and more attractive reports as well) @Bauer2023-eh.

Whether Rmarkdown, Jupyter or Quarto, literate programming offers some substantial benefits over alternative
approaches:
...
copy-paste operations required might contain errors.
...
By arranging the outputs (eg: charts and tables) in a document in the sequence that they were generated,
...
so we are not sure whether the whole script is free of errors.
