...
To keep the work available and accessible at all times, google cloud storage is used to store several types of files. A google spreadsheet is maintained to 
track the progress. The sheet contains details like PMCID, type of organism, availability of gene set, usage of DAVID, and details about the utilization of the background gene list by the authors. The Google spreadsheet is attached as supplementary file 1 named as “Anusuiya_PMC_2019”. 

The articles are searched for the dataset of dysregulated (upregulated and downregulated, either combined or separately) genes. It is important to understand 
how the list of specific gene lists was obtained by the researchers. This is usually mentioned in the materials and methods section of the paper. The list of 
genes found in the articles is stored in the spreadsheet named “Anusuiya_gene_list”. The gene list is maintained for our analysis under their respective PMCIDs.
Special attention is given to detecting gene name errors that are usually in converted date formats when procured from excel sheets from the original paper. On
request, the gene list file can be shared.

The reproducibility analysis is performed using the respective methodologies stated by the 20 articles using DAVID. It is important to note that the DAVID 6.8 
server has been replaced by DAVID 2021 server, starting from June 1, 2022. For our analysis, DAVID 6.8 version was used for replication procedures. It was 
ensured that our analyses got completed by 31 May 2022. 
...
14. Select the terms up to the prescribed P Value criteria from the paper. Other options like FDR values, Fischer Test values can be obtained by expanding 
“options” above the results table and clicking on the required options. Click on Rerun using options.
...
A live google document is used to store all the results that consisted of Gene ontology terms (Biological Process, Cellular Component, and Molecular Function) 
and KEGG Pathway results. Terms and pathway results are included till the mentioned P values or corrected P Values from the paper (generally, P<0.005). 

Each article is re-read, keeping note of sections like Abstract, Results, Discussion, and Conclusion. It is checked whether the DAVID results obtained by the
researchers are used in making the conclusions mentioned in the respective papers. This gives an idea of whether DAVID results were included to form conclusions 
by the authors. 
...
* Paper is rated “3” when it shows maximum reproducibility. The terms upon replication are observed again according to the ones mentioned in the paper. The 
conclusion of the paper is comprehensible and accurate.
...
A summary table is created that includes information about the parts of the paper that are reproduced using DAVID. It also states if replication analysis is 
possible with subsequent reasoning. 
